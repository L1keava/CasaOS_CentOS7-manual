#!/usr/bin/bash
#
#           CasaOS Installer Script v0.4.11#
...
readonly CASA_UNINSTALL_URL="https://get.casaos.io/uninstall/v0.4.11"
...
"${CASA_DOWNLOAD_DOMAIN}IceWhaleTech/CasaOS/releases/download/v0.4.11/linux-${TARGET_ARCH}-casaos-v0.4.11.tar.gz"
...
"${CASA_DOWNLOAD_DOMAIN}IceWhaleTech/CasaOS-UI/releases/download/v0.4.10/linux-all-casaos-v0.4.10.tar.gz"
...
DownloadAndInstallCasaOS() {
    if [ -z "${BUILD_DIR}" ]; then
        ...
        for PACKAGE in "${CASA_PACKAGES[@]}"; do
            ...
            GreyStart
            ${sudo_cmd} wget "${PACKAGE}" || Show 1 "Failed to download package"
            ColorReset
        done
